...
import { formatYear, tunnelTwist, wormholeGridLines, wormholeRings, type WormholeState } from '@/lib/wormhole-layout';
    ...
    <g aria-label="Zeit-Wurmloch">
      <defs>
        <radialGradient id="wormhole-vignette" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#f7f7f4" stopOpacity="0.18" />
          <stop offset="45%" stopColor="#f7f7f4" stopOpacity="0.05" />
      ...
      {rings.map((ring, index) => {
        const depth = Math.max(0, Math.min(1, 1 - (ring.radius - 34) / (620 - 34)));
        const labelPoint = polarToCartesian(atlasSize.cx, atlasSize.cy, ring.radius, 102 + tunnelTwist(depth, state.phase));
        const labelScale = Math.max(0.55, 1.25 - depth);
        const labelOpacity = ring.mode === 'local' ? 0.92 : Math.max(0.2, 1 - depth * 0.92);
        ...
        return (
          <g key={`${ring.year}-${index}`}>
            <circle
              ...
              opacity={ring.mode === 'local' ? 0.84 : Math.max(0.13, 0.5 - depth * 0.36)}
            />
            <text
              x={labelPoint.x}
              y={labelPoint.y}
