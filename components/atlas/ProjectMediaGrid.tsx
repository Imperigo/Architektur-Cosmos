  ...
  return (
    <g>
      {types.map((type, index) => {
        ...
        return (
          <g key={type}>
            <rect x={slotX} y={slotY} width={slotWidth} height={slotHeight} fill="#050505" stroke="#f7f7f4" strokeWidth="0.8" />
            <MediaPlaceholder type={type} x={slotX} y={slotY} width={slotWidth} height={slotHeight} />
            {showLabels ? (
              <text x={slotX} y={slotY + slotHeight + 10} fill="#b8b8b8" fontSize="8" fontFamily="var(--font-sans), system-ui, sans-serif" letterSpacing="0.08em">
...
function MediaPlaceholder({ type, x, y, width, height }: { type: EntryMediaType; x: number; y: number; width: number; height: number }) {
  if (type === 'exterior') {
    return (
      <g stroke="#f7f7f4" strokeWidth="0.8" fill="none" opacity="0.84">
  ...
  if (type === 'interior') {
    return (
      <g stroke="#f7f7f4" strokeWidth="0.75" fill="none" opacity="0.78">
  ...
  if (type === 'section') {
    return (
      <g stroke="#f7f7f4" strokeWidth="0.75" fill="none" opacity="0.82">
