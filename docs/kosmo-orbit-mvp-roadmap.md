...
14. Local-Identity-Vertrag fuer spaetere lokale Profile, Auth und Sessions
...
16. Office-Memory-Readiness-Vertrag fuer Projektkontext, Entscheide,
    Asset-Evidenz, Ausbildung und Betrieb als spaeteres lokales
...
18. Delete-/Export-/Restore-Drill fuer Loeschanfrage, Exportpaket,
    Restore-Probe und Audit-Spur, bevor echte Delete-/Export-/Restore-Jobs,
    Backup-Restore, Retention-Automation oder externer Archiv-Sync erlaubt
    werden
    (**initial umgesetzt mit `npm run kosmo:orbit-delete-export-restore-drill` und `/orbit`**)
19. spaeter Handoff an KosmoZentrale fuer echte lokale Runtime
...
- `npm run kosmo:orbit-office-pilot-scene`
...
- automatisiert die sichere lokale Reihenfolge Office-Pilot-Scene-Check,
  Route-Smoke, Static-Smoke, Cleanup, Push-Readiness und Cleanup;
...
- Pilot-Result-Draft sichtbar in `/orbit` mit leeren `null`-Slots,
  fehlender Evidenz und blockierten Public Claims;
...
- `npm run kosmo:orbit-route-smoke` — 253/253 passed;
