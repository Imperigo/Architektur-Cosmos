...
Generated: 2026-06-30T06:54:33.501Z
Status: `owner_review_session_brief_guard_failed`
...
- failure: `packet_guard_passed` - Session brief must reference a passing packet guard.
- passed: `router_guarded_review_only` - Session brief must keep router guarded review-only or private-diagnostic ready.
...
- passed: `source_root_decision_pending` - Source-root decision must remain pending or recorded metadata-only.
- passed: `source_root_path_pending` - Source-root path must remain pending or recorded metadata-only.
- passed: `source_root_safe_default_keep_blocked` - Source-root safe default must remain keep_blocked or metadata-only after recorded root.
