...
- Source-root decision dry run: source_root_decision_dry_run_needs_review, scenarios 3, metadata scenarios 1, failures 1
- Source-root post-owner activation queue: source_root_post_owner_activation_queue_needs_review, steps 7, executable 0, blocked 7, failures 3
- Source-root post-owner activation queue check: source_root_post_owner_activation_queue_guard_failed, failures 2, warnings 0
- Source-root owner final decision brief: source_root_owner_final_decision_brief_needs_review, options 3, unlock options 1, failures 1
- Source-root owner choice consequence matrix: source_root_owner_choice_consequence_matrix_needs_review, choices 3, unlock 1, blocked 2, failures 3
...
- Private metadata inventory check: private_metadata_inventory_guard_failed
...
- Local worker fixture chain task pack: local_worker_fixture_chain_task_pack_missing_refs, tasks 3, executable 0, missing refs 6, check local_worker_fixture_chain_task_pack_guard_failed, failures 8
...
| --- | --- | --- | --- |
| `day-batch` Daily Batch | needs_review | no | 12/13 required steps |
...
| `source-root-decision-dry-run` Source Root Decision Dry Run | needs_review | no | 3 scenarios, metadata 1, failures 1 |
| `source-root-post-owner-activation-queue` Source Root Post-Owner Activation Queue | needs_review | no | 7 steps, executable 0, blocked 7 |
...
| `fixture-chain-local-worker-task-pack` Fixture Chain Local Worker Task Pack | needs_review | no | 3 tasks, GitHub 0, payloads 0, training 0, executable now 0, missing refs 6 |
...
| `github-innovation-discovery` GitHub Innovation Discovery | review_only_ready | no | 5/10 queries with results, 25 candidates, failures 0 |
| `github-innovation-review-queue` GitHub Innovation Review Queue | review_only_ready | no | 9 review items, high 5, execute 0, failures 0 |
| `codex-morning-routine-run` Codex Morning Routine Run | review_only_ready | no | fetch 2/2, behind 0, handoff 352, next source_free_innovation_and_guarding, failures 0 |
| `today-loop-plan` Today Loop Plan | needs_review | no | -, blocks 0, tick -m, checkup -m, failures 0 |
| `github-readme-signal-scan` GitHub README Signal Scan | review_only_ready | no | 9 scanned, README 8, high-signal 6, failures 0 |
| `github-fixture-contract-plan` GitHub Fixture Contract Plan | review_only_ready | no | 6 plans, prepare 2, asset 1, worker 3, failures 0 |
