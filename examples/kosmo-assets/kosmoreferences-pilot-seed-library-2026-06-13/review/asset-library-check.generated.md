# KosmoAsset Library Check

Library: `kosmoreferences-pilot-seed-library-2026-06-13`
Generated: 2026-06-13T18:23:42.377Z
Status: `passed`

This is a local review-only check. It does not upload assets, write D1/R2 or publish public downloads.

## Summary

- assets: 6
- local ready: 0
- public ready: 0
- planned only: 6
- KosmoData refs: 6
- failures: 0
- warnings: 0

## Assets

| Asset | Type | Preview | Rights | Review | Formats | KosmoData | Local | Public |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Villa Savoye Concrete Frame Material Study | material | material_swatch | generated_needs_review | planned | material_json | 1 | no | no |
| Villa Savoye Five Points Diagram Kit | detail | wireframe_component | generated_needs_review | planned | svg, json | 1 | no | no |
| Sogn Benedetg Wood Shingle Material Study | material | material_swatch | generated_needs_review | needs_source | material_json | 1 | no | no |
| Sogn Benedetg Light Band Detail Study | detail | wireframe_component | generated_needs_review | needs_source | dxf, svg | 1 | no | no |
| Ingenbohl Mineral Pigment Material Study | material | material_swatch | generated_needs_review | planned | material_json | 1 | no | no |
| Ingenbohl Concrete Core and Frame Study | component | wireframe_component | generated_needs_review | needs_source | json, glb | 1 | no | no |

## KosmoData Bridge

| Asset | KosmoData entry | Relation | Usage | Review |
| --- | --- | --- | --- | --- |
| Villa Savoye Concrete Frame Material Study | Villa Savoye | material_context | derived_asset_review_required | needs_human_review |
| Villa Savoye Five Points Diagram Kit | Villa Savoye | typology_context | context_only | context_only |
| Sogn Benedetg Wood Shingle Material Study | Kapelle Sogn Benedetg | material_context | derived_asset_review_required | needs_human_review |
| Sogn Benedetg Light Band Detail Study | Kapelle Sogn Benedetg | typology_context | context_only | context_only |
| Ingenbohl Mineral Pigment Material Study | Alterszentrum Kloster Ingenbohl | material_context | derived_asset_review_required | needs_human_review |
| Ingenbohl Concrete Core and Frame Study | Alterszentrum Kloster Ingenbohl | model_context | derived_asset_review_required | needs_human_review |

## Failures

- None.

## Warnings

- None.

## Next Actions

- Generate or attach reviewed local files for planned-only assets.
