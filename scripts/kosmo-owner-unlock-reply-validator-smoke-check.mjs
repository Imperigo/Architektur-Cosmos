...
function buildChecks(smoke) {
  ...
  return [
    ...
    check('four_cases', cases.length === 4, cases.map((smokeCase) => smokeCase.id).join(',')),
    ...
    check('one_expected_invalid_case', smoke.summary?.expected_invalid_cases === 1, smoke.summary?.expected_invalid_cases),
