...
function buildReport(html) {
  ...
  const checks = [
    ...
    check('renders_kosmosketch_adapter', 'Export renders KosmoSketch tool adapter contract.', html.includes('KosmoSketch ToolAdapter') && html.includes('Target-Tool Vertrag fuer Skizze zu BIM') && html.includes('kosmo-draw.kosmosketch')),
    ...
    check('renders_delete_export_restore_drill', 'Export renders delete/export/restore drill.', html.includes('Delete / Export / Restore Drill') && html.includes('Wie KosmoOrbit lokale Daten reversibel und pruefbar halten muss')),
    ...
    check('keeps_kosmosketch_adapter_review_only', 'Export keeps KosmoSketch backend, approval, artifact, Blender, BIM, IFC and 2D actions blocked.', html.includes('kein POST /jobs') && html.includes('kein /router/plan') && html.includes('keine Approval-Mutation') && html.includes('kein Artifact-Upload') && html.includes('kein Blender-Start') && html.includes('kein BIM-Commit') && html.includes('kein IFC-Export') && html.includes('keine 2D-Regeneration')),
    ...
    check('keeps_delete_export_restore_review_only', 'Export keeps delete, export, restore, customer data, backup restore and external archive sync blocked.', html.includes('kein real delete job') && html.includes('kein real export job') && html.includes('kein real restore job') && html.includes('kein Kundendaten-Export') && html.includes('kein Kundendaten-Delete') && html.includes('kein Backup-Restore') && html.includes('kein externer Archiv-Sync')),
