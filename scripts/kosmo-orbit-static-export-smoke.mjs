...
function buildReport(html) {
  ...
  const checks = [
    ...
    check('anchors_core_sections', 'Export contains section anchors.', ['autonomie', 'routine', 'workflow-delta', 'pilotmessung', 'fortschritt', 'vision', 'demo-ready', 'projektpaket', 'design-handoff', 'entscheidung', 'runtime', 'runtime-contract', 'installation', 'health', 'risiken', 'commands', 'audit', 'evidenz', 'ausbildung', 'rechte', 'rollen', 'guardrails'].every((id) => html.includes(`id="${id}"`))),
