...
function buildCertificate({ library, asset, route, decisionPath, humanSession, humanSessionAsset, reviewPack, handoffBundle, handoffSmoke }) {
  ...
  return {
    ...
    route,
    certificate_title: 'Architecture Kosmos Local Quality Certificate V1',
    ...
    quality_scope: {
      human_architecture_review: true,
      source_and_rights_review: true,
      ai_slop_quality_gate: true,
      local_sandbox_evidence_only: true,
      public_release_approval: false,
      official_external_certification: false
    },
    quality_statement: [
...
function renderMarkdown(report) {
  const lines = [
    ...
    '',
    `${report.certificate_title}. This is local review evidence only. It does not approve public use, upload assets, write D1/R2, mutate the library, write Blender scenes or write ArchiCAD project files.`,
    '',
    '## Summary',
    '',
    ...
    '',
    `- human architecture review: ${report.quality_scope.human_architecture_review ? 'yes' : 'no'}`,
    `- source and rights review: ${report.quality_scope.source_and_rights_review ? 'yes' : 'no'}`,
    `- AI slop quality gate: ${report.quality_scope.ai_slop_quality_gate ? 'yes' : 'no'}`,
    `- public release approval: ${report.quality_scope.public_release_approval ? 'yes' : 'no'}`,
    `- official external certification: ${report.quality_scope.official_external_certification ? 'yes' : 'no'}`,
    '',
    '## Limitations',
    '',
    ...report.quality_statement.map((item) => `- ${item}`),
    '',
    '## Evidence',
    '',
