...
const toolchainReadinessPath = resolve(root, args.toolchainReadiness || 'app/orbit/OrbitToolchainReadiness.tsx');
const toolchainReadinessContractPath = resolve(root, args.toolchainReadinessContract || 'examples/kosmo-orbit/health/orbit-toolchain-readiness.contract.json');
...
const autonomousFireStatePath = resolve(root, args.autonomousFireState || 'app/orbit/OrbitAutonomousFireState.tsx');
const autonomousFireStateDataPath = resolve(root, args.autonomousFireStateData || 'examples/kosmo-orbit/memory/orbit-autonomous-fire-state.contract.json');
const autonomousLoopLedgerPath = resolve(root, args.autonomousLoopLedger || 'app/orbit/OrbitAutonomousLoopLedger.tsx');
const autonomousLoopLedgerDataPath = resolve(root, args.autonomousLoopLedgerData || 'examples/kosmo-orbit/memory/orbit-autonomous-loop-ledger.contract.json');
const fireCadenceGuardPath = resolve(root, args.fireCadenceGuard || 'app/orbit/OrbitFireCadenceGuard.tsx');
const fireCadenceGuardDataPath = resolve(root, args.fireCadenceGuardData || 'examples/kosmo-orbit/memory/orbit-fire-cadence-guard.contract.json');
...
const githubImperigoGatePath = resolve(root, args.githubImperigoGate || 'app/orbit/OrbitGitHubImperigoGate.tsx');
const githubImperigoGateDataPath = resolve(root, args.githubImperigoGateData || 'examples/kosmo-orbit/governance/orbit-github-imperigo-gate.contract.json');
const pushDecisionDraftPath = resolve(root, args.pushDecisionDraft || 'app/orbit/OrbitPushDecisionDraft.tsx');
const pushDecisionDraftDataPath = resolve(root, args.pushDecisionDraftData || 'examples/kosmo-orbit/governance/orbit-push-decision-draft.contract.json');
...
async function main() {
  ...
  const toolchainReadinessSource = existsSync(toolchainReadinessPath) ? readFileSync(toolchainReadinessPath, 'utf8') : '';
  const toolchainReadinessContractSource = existsSync(toolchainReadinessContractPath) ? readFileSync(toolchainReadinessContractPath, 'utf8') : '';
  ...
  const autonomousFireStateSource = existsSync(autonomousFireStatePath) ? readFileSync(autonomousFireStatePath, 'utf8') : '';
  const autonomousFireStateDataSource = existsSync(autonomousFireStateDataPath) ? readFileSync(autonomousFireStateDataPath, 'utf8') : '';
  const autonomousLoopLedgerSource = existsSync(autonomousLoopLedgerPath) ? readFileSync(autonomousLoopLedgerPath, 'utf8') : '';
  const autonomousLoopLedgerDataSource = existsSync(autonomousLoopLedgerDataPath) ? readFileSync(autonomousLoopLedgerDataPath, 'utf8') : '';
  const fireCadenceGuardSource = existsSync(fireCadenceGuardPath) ? readFileSync(fireCadenceGuardPath, 'utf8') : '';
  const fireCadenceGuardDataSource = existsSync(fireCadenceGuardDataPath) ? readFileSync(fireCadenceGuardDataPath, 'utf8') : '';
  ...
  const githubImperigoGateSource = existsSync(githubImperigoGatePath) ? readFileSync(githubImperigoGatePath, 'utf8') : '';
  const githubImperigoGateDataSource = existsSync(githubImperigoGateDataPath) ? readFileSync(githubImperigoGateDataPath, 'utf8') : '';
  const pushDecisionDraftSource = existsSync(pushDecisionDraftPath) ? readFileSync(pushDecisionDraftPath, 'utf8') : '';
  const pushDecisionDraftDataSource = existsSync(pushDecisionDraftDataPath) ? readFileSync(pushDecisionDraftDataPath, 'utf8') : '';
  const sectionIndexSource = existsSync(sectionIndexPath) ? readFileSync(sectionIndexPath, 'utf8') : '';
  const spec = readJson(specPath);
  const report = buildReport({ routeSource, roleSwitcherSource, demoReviewSource, projectDashboardSource, designHandoffPanelSource, designPilotSource, designHandoffPanelDataSource, presenterBriefSource, workflowDeltaSource, pilotMeasurementSource, pilotMeasurementKitSource, pilotFacilitatorChecklistSource, pilotResultDraftSource, pilotRunbookSource, pilotSessionTemplateSource, progressMapSource, visionBridgeSource, demoQuestionsSource, installationTopologySource, healthReadinessSource, healthReadinessContractSource, toolchainReadinessSource, toolchainReadinessContractSource, riskRegisterSource, commandContractSource, commandContractDataSource, auditTrailSource, auditTrailDataSource, reviewDecisionDraftSource, runtimeBoundarySource, runtimeAdapterSource, runtimeAdapterDataSource, kosmoSketchAdapterSource, kosmoSketchAdapterDataSource, runtimeContractSource, qualityEvidenceSource, workstationPrioritiesSource, workstationProfileSource, workstationProfileDataSource, localIdentitySource, localIdentityDataSource, dataGovernanceSource, dataGovernanceDataSource, officeMemorySource, officeMemoryDataSource, localStorageDecisionSource, localStorageDecisionDataSource, deleteExportRestoreSource, deleteExportRestoreDataSource, learningModeSource, permissionMatrixSource, autonomyStatusSource, autonomousFireStateSource, autonomousFireStateDataSource, autonomousLoopLedgerSource, autonomousLoopLedgerDataSource, fireCadenceGuardSource, fireCadenceGuardDataSource, officeRoutineSource, officeRoutineDataSource, officePilotSceneSource, officePilotSceneDataSource, toolRegistrySource, toolRegistryDataSource, demoReadinessSource, publishReadinessSource, launchDecisionBriefSource, githubImperigoGateSource, githubImperigoGateDataSource, pushDecisionDraftSource, pushDecisionDraftDataSource, sectionIndexSource, spec });
...
function buildReport({ routeSource, roleSwitcherSource, demoReviewSource, projectDashboardSource, designHandoffPanelSource, designPilotSource, designHandoffPanelDataSource, presenterBriefSource, workflowDeltaSource, pilotMeasurementSource, pilotMeasurementKitSource, pilotFacilitatorChecklistSource, pilotResultDraftSource, pilotRunbookSource, pilotSessionTemplateSource, progressMapSource, visionBridgeSource, demoQuestionsSource, installationTopologySource, healthReadinessSource, healthReadinessContractSource, toolchainReadinessSource, toolchainReadinessContractSource, riskRegisterSource, commandContractSource, commandContractDataSource, auditTrailSource, auditTrailDataSource, reviewDecisionDraftSource, runtimeBoundarySource, runtimeAdapterSource, runtimeAdapterDataSource, kosmoSketchAdapterSource, kosmoSketchAdapterDataSource, runtimeContractSource, qualityEvidenceSource, workstationPrioritiesSource, workstationProfileSource, workstationProfileDataSource, localIdentitySource, localIdentityDataSource, dataGovernanceSource, dataGovernanceDataSource, officeMemorySource, officeMemoryDataSource, localStorageDecisionSource, localStorageDecisionDataSource, deleteExportRestoreSource, deleteExportRestoreDataSource, learningModeSource, permissionMatrixSource, autonomyStatusSource, autonomousFireStateSource, autonomousFireStateDataSource, autonomousLoopLedgerSource, autonomousLoopLedgerDataSource, fireCadenceGuardSource, fireCadenceGuardDataSource, officeRoutineSource, officeRoutineDataSource, officePilotSceneSource, officePilotSceneDataSource, toolRegistrySource, toolRegistryDataSource, demoReadinessSource, publishReadinessSource, launchDecisionBriefSource, githubImperigoGateSource, githubImperigoGateDataSource, pushDecisionDraftSource, pushDecisionDraftDataSource, sectionIndexSource, spec }) {
  ...
  const checks = [
    ...
    check('toolchain_readiness_file_exists', 'Orbit toolchain readiness component exists.', existsSync(toolchainReadinessPath)),
    check('toolchain_readiness_contract_file_exists', 'Orbit toolchain readiness contract exists.', existsSync(toolchainReadinessContractPath)),
    ...
    check('autonomous_fire_state_file_exists', 'Orbit autonomous fire state component exists.', existsSync(autonomousFireStatePath)),
    check('autonomous_fire_state_data_file_exists', 'Orbit autonomous fire state contract exists.', existsSync(autonomousFireStateDataPath)),
    check('autonomous_loop_ledger_file_exists', 'Orbit autonomous loop ledger component exists.', existsSync(autonomousLoopLedgerPath)),
    check('autonomous_loop_ledger_data_file_exists', 'Orbit autonomous loop ledger contract exists.', existsSync(autonomousLoopLedgerDataPath)),
    check('fire_cadence_guard_file_exists', 'Orbit fire cadence guard component exists.', existsSync(fireCadenceGuardPath)),
    check('fire_cadence_guard_data_file_exists', 'Orbit fire cadence guard contract exists.', existsSync(fireCadenceGuardDataPath)),
    ...
    check('github_imperigo_gate_file_exists', 'Orbit GitHub Imperigo gate component exists.', existsSync(githubImperigoGatePath)),
    check('github_imperigo_gate_data_file_exists', 'Orbit GitHub Imperigo gate contract exists.', existsSync(githubImperigoGateDataPath)),
    check('push_decision_draft_file_exists', 'Orbit push decision draft component exists.', existsSync(pushDecisionDraftPath)),
    check('push_decision_draft_data_file_exists', 'Orbit push decision draft contract exists.', existsSync(pushDecisionDraftDataPath)),
    ...
    check('imports_toolchain_readiness', 'Route imports the toolchain readiness component.', routeSource.includes('OrbitToolchainReadiness')),
    check('imports_toolchain_readiness_contract', 'Toolchain readiness component imports the local contract JSON.', source.includes('orbit-toolchain-readiness.contract.json')),
    check('imports_heavy_check_timebox_report', 'Toolchain readiness component imports the heavy-check timebox report.', source.includes('orbit-heavy-check-timebox.generated.json')),
    ...
    check('imports_autonomous_fire_state', 'Route imports the autonomous fire state component.', routeSource.includes('OrbitAutonomousFireState')),
    check('imports_autonomous_fire_state_data', 'Autonomous fire state component imports the local contract JSON.', source.includes('orbit-autonomous-fire-state.contract.json')),
    check('imports_autonomous_loop_ledger', 'Route imports the autonomous loop ledger component.', routeSource.includes('OrbitAutonomousLoopLedger')),
    check('imports_autonomous_loop_ledger_data', 'Autonomous loop ledger component imports the local contract JSON.', source.includes('orbit-autonomous-loop-ledger.contract.json')),
    check('imports_fire_cadence_guard', 'Route imports the fire cadence guard component.', routeSource.includes('OrbitFireCadenceGuard')),
    check('imports_fire_cadence_guard_data', 'Fire cadence guard component imports the local contract JSON.', source.includes('orbit-fire-cadence-guard.contract.json')),
    ...
    check('imports_github_imperigo_gate', 'Route imports the GitHub Imperigo gate component.', routeSource.includes('OrbitGitHubImperigoGate')),
    check('imports_github_imperigo_gate_data', 'GitHub Imperigo gate component imports the local contract JSON.', source.includes('orbit-github-imperigo-gate.contract.json')),
    check('imports_push_decision_draft', 'Route imports the push decision draft component.', routeSource.includes('OrbitPushDecisionDraft')),
    check('imports_push_decision_draft_data', 'Push decision draft component imports the local contract JSON.', source.includes('orbit-push-decision-draft.contract.json')),
    ...
    check('shows_toolchain_readiness', 'Route renders toolchain readiness and heavy-check lanes.', source.includes('Toolchain Readiness') && source.includes('Schwere Checks als klares Release-Gate') && source.includes('Next Static Build')),
    check('keeps_toolchain_release_safe', 'Toolchain readiness blocks false green release claims, static smoke without build, push and deploy.', source.includes('kein falsches Gruen') && source.includes('kein Static-Smoke ohne Build') && source.includes('kein Push') && source.includes('kein Deploy')),
    ...
    check('keeps_autonomy_named_orbit', 'Autonomy status names KosmoOrbit, not KosmoWebsite.', autonomyStatusSource.includes('Was KosmoOrbit gerade selbststaendig tut') && !autonomyStatusSource.includes('KosmoWebsite')),
    check('shows_autonomous_fire_state', 'Route renders the autonomous fire state.', source.includes('Autonomer Fire State') && source.includes('5-Minuten-Fire') && source.includes('Autonomer Block bis 24:00 Zuerich Zeit')),
    check('autonomous_fire_uses_local_contract', 'Autonomous fire state uses the local contract for permissions, addon memory, own memory and blocked actions.', source.includes('autonomousFireStateData') && source.includes('addon_memory') && source.includes('own_memory') && source.includes('blocked_today')),
    check('keeps_autonomous_fire_safe', 'Autonomous fire state keeps push, deploy, external accounts and runtime writes blocked.', source.includes('kein Push') && source.includes('kein Deploy') && source.includes('keine externen Accounts') && source.includes('keine Runtime-Writes')),
    check('shows_autonomous_loop_ledger', 'Route renders the autonomous loop ledger summary.', source.includes('Autonomous Loop Ledger') && source.includes('Gesamtzusammenfassung') && source.includes('Fire Timeline')),
    check('autonomous_loop_ledger_uses_local_contract', 'Autonomous loop ledger uses local contract for fire records, memory additions and blocked boundaries.', source.includes('loopLedgerData') && source.includes('fire_records') && source.includes('memory_added_today') && source.includes('still_blocked')),
    check('keeps_autonomous_loop_ledger_safe', 'Autonomous loop ledger keeps owner-go, heavy-check and external boundaries visible.', source.includes('push_without_owner_go') && source.includes('typescript_green_claim_without_completed_logs') && source.includes('external_account_change')),
    check('shows_fire_cadence_guard', 'Route renders the fire cadence guard and 5-minute drift copy.', source.includes('Fire Cadence Guard') && source.includes('5-Minuten-Takt') && source.includes('drifted')),
    check('fire_cadence_guard_uses_local_contract', 'Fire cadence guard uses the local contract for observed fires, policy and blocked boundaries.', source.includes('cadenceGuardData') && source.includes('observed_fires') && source.includes('cadence_policy') && source.includes('blocked_today')),
    check('keeps_fire_cadence_guard_safe', 'Fire cadence guard blocks daemons, external schedulers, push, deploy and hidden drift.', source.includes('kein Daemon') && source.includes('kein externer Scheduler') && source.includes('kein Push') && source.includes('kein Deploy') && source.includes('kein Verstecken von Drift')),
    ...
    check('shows_github_imperigo_gate', 'Route renders GitHub Imperigo gate and 5-minute fire copy.', source.includes('GitHub Imperigo Gate') && source.includes('Automation sichtbar, externe Wirkung gesperrt') && source.includes('5-Minuten-Fire')),
    check('keeps_github_imperigo_safe', 'GitHub Imperigo gate blocks push, deploy, GitHub mutation and secrets.', source.includes('kein Push') && source.includes('kein Deploy') && source.includes('keine GitHub-Mutation') && source.includes('keine Secrets')),
    check('shows_push_decision_draft', 'Route renders push decision draft and hold-local copy.', source.includes('Push Decision Draft') && source.includes('Lokaler Entscheid, kein Push') && source.includes('hold_local')),
    check('keeps_push_decision_safe', 'Push decision draft blocks push, deploy, GitHub mutation, secrets and costs.', source.includes('kein Push') && source.includes('kein Deploy') && source.includes('keine GitHub-Mutation') && source.includes('keine Secrets') && source.includes('keine Kostenjobs')),
    check('shows_section_index', 'Route renders compact demo section navigation.', source.includes('Demo-Navigation') && source.includes('#workflow-delta') && source.includes('#pilotmessung') && source.includes('#pilot-kit') && source.includes('#pilot-checklist') && source.includes('#pilot-result') && source.includes('#pilotplan') && source.includes('#pilot-session') && source.includes('#fortschritt') && source.includes('#vision') && source.includes('#runtime-contract') && source.includes('#runtime-adapter') && source.includes('#kosmosketch-adapter') && source.includes('#installation') && source.includes('#health') && source.includes('#toolchain-readiness') && source.includes('#risiken') && source.includes('#commands') && source.includes('#audit') && source.includes('#demo-ready') && source.includes('#live-gate') && source.includes('#launch-brief') && source.includes('#github-imperigo-gate') && source.includes('#push-decision-draft') && source.includes('#office-pilot') && source.includes('#tool-registry') && source.includes('#autonomous-fire') && source.includes('#autonomous-loop-ledger') && source.includes('#fire-cadence-guard') && source.includes('#routine') && source.includes('#workstation-profile') && source.includes('#local-identity') && source.includes('#data-governance') && source.includes('#office-memory') && source.includes('#local-storage-decision') && source.includes('#delete-export-restore') && source.includes('#ausbildung') && source.includes('#rechte') && source.includes('#projektpaket') && source.includes('#design-handoff') && source.includes('#design-pilot') && source.includes('#guardrails')),
    check('anchors_core_sections', 'Route contains anchors for core demo sections.', source.includes('id="autonomie"') && source.includes('id="autonomous-fire"') && source.includes('id="autonomous-loop-ledger"') && source.includes('id="fire-cadence-guard"') && source.includes('id="routine"') && source.includes('id="tool-registry"') && source.includes('id="workflow-delta"') && source.includes('id="pilotmessung"') && source.includes('id="pilot-kit"') && source.includes('id="pilot-checklist"') && source.includes('id="pilot-result"') && source.includes('id="pilotplan"') && source.includes('id="pilot-session"') && source.includes('id="vision"') && source.includes('id="demo-ready"') && source.includes('id="live-gate"') && source.includes('id="launch-brief"') && source.includes('id="github-imperigo-gate"') && source.includes('id="push-decision-draft"') && source.includes('id="office-pilot"') && source.includes('id="runtime-contract"') && source.includes('id="runtime-adapter"') && source.includes('id="kosmosketch-adapter"') && source.includes('id="installation"') && source.includes('id="health"') && source.includes('id="toolchain-readiness"') && source.includes('id="risiken"') && source.includes('id="commands"') && source.includes('id="audit"') && source.includes('id="projektpaket"') && source.includes('id="design-handoff"') && source.includes('id="design-pilot"') && source.includes('id="workstation-profile"') && source.includes('id="local-identity"') && source.includes('id="data-governance"') && source.includes('id="office-memory"') && source.includes('id="local-storage-decision"') && source.includes('id="delete-export-restore"') && source.includes('id="ausbildung"') && source.includes('id="rechte"') && source.includes('id="rollen"')),
