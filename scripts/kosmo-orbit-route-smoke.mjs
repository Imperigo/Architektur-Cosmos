...
function buildReport({ routeSource, roleSwitcherSource, demoReviewSource, projectDashboardSource, designHandoffPanelSource, designPilotSource, designHandoffPanelDataSource, presenterBriefSource, workflowDeltaSource, pilotMeasurementSource, pilotMeasurementKitSource, pilotFacilitatorChecklistSource, pilotResultDraftSource, pilotRunbookSource, pilotSessionTemplateSource, progressMapSource, visionBridgeSource, demoQuestionsSource, installationTopologySource, healthReadinessSource, healthReadinessContractSource, riskRegisterSource, commandContractSource, commandContractDataSource, auditTrailSource, auditTrailDataSource, reviewDecisionDraftSource, runtimeBoundarySource, runtimeAdapterSource, runtimeAdapterDataSource, runtimeContractSource, qualityEvidenceSource, workstationPrioritiesSource, learningModeSource, permissionMatrixSource, autonomyStatusSource, officeRoutineSource, officeRoutineDataSource, officePilotSceneSource, officePilotSceneDataSource, toolRegistrySource, toolRegistryDataSource, demoReadinessSource, publishReadinessSource, launchDecisionBriefSource, sectionIndexSource, spec }) {
  ...
  const checks = [
    ...
    check('shows_section_index', 'Route renders compact demo section navigation.', source.includes('Demo-Navigation') && source.includes('#workflow-delta') && source.includes('#pilotmessung') && source.includes('#pilot-kit') && source.includes('#pilot-checklist') && source.includes('#pilot-result') && source.includes('#pilotplan') && source.includes('#pilot-session') && source.includes('#fortschritt') && source.includes('#vision') && source.includes('#runtime-contract') && source.includes('#runtime-adapter') && source.includes('#installation') && source.includes('#health') && source.includes('#risiken') && source.includes('#commands') && source.includes('#audit') && source.includes('#demo-ready') && source.includes('#live-gate') && source.includes('#launch-brief') && source.includes('#office-pilot') && source.includes('#tool-registry') && source.includes('#routine') && source.includes('#ausbildung') && source.includes('#rechte') && source.includes('#projektpaket') && source.includes('#design-handoff') && source.includes('#design-pilot') && source.includes('#guardrails')),
