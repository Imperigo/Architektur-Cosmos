...
const outputJson = resolve(root, args.out || `data/kosmo-innovation-github-fixture-skeletons-check-${dateStamp}.json`);
const outputMd = resolve(root, args.markdown || `docs/codex/kosmo-innovation-github-fixture-skeletons-check-${dateStamp}.md`);
...
async function main() {
  ...
  const report = {
    ...
    status: failures.length === 0 ? 'innovation_github_fixture_skeletons_guard_passed' : 'innovation_github_fixture_skeletons_guard_failed',
    policy: {
    ...
    next_actions: failures.length === 0
      ...
      : [
          'Fix GitHub fixture skeleton guard failures.',
          'Rerun npm run kosmo:innovation-github-fixture-skeletons and this guard.'
        ]
  };
  ...
  console.log('Kosmo innovation GitHub fixture skeletons check');
...
async function checkSkeletons(report) {
  ...
  expect(report.status === 'innovation_github_fixture_skeletons_ready', findings, 'skeletons_ready', 'Skeleton report must be ready.');
  ...
  for (const file of report.written_files || []) {
    ...
    if (file.endsWith('fixture-manifest.json')) {
      ...
      expect(manifest.status === 'github_signal_fixture_skeleton_only', findings, `manifest_status:${file}`, `${file} must be github_signal_fixture_skeleton_only.`);
      expect(manifest.source_repo_is_reference_only === true, findings, `manifest_repo_ref:${file}`, `${file} must mark the source repo as reference only.`);
      expect(manifest.policy?.synthetic_fixture_only === true, findings, `manifest_synthetic:${file}`, `${file} must be synthetic-fixture-only.`);
      expect(manifest.policy?.github_reference_only === true, findings, `manifest_github_ref:${file}`, `${file} must mark GitHub as reference only.`);
      expect(manifest.policy?.copied_github_code === false, findings, `manifest_no_code_copy:${file}`, `${file} must not copy GitHub code.`);
      expect(manifest.policy?.copied_readme_text === false, findings, `manifest_no_readme_copy:${file}`, `${file} must not copy README text.`);
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# Kosmo Innovation GitHub Fixture Skeletons Check');
  lines.push('');
