...
const tempRoot = resolve(root, args['tmp-dir'] || '.tmp/public-demo-gate-static-sentinel-negative-smoke');
const keepTemp = Boolean(args['keep-temp']);
const expectedFailure = 'static-route:/references/:includes:Medienfreigabe';
...
function runSmoke() {
  ...
  if (result.status === 0) {
    throw new Error('Expected public-demo-gate-check to fail for a synthetic exported route with a missing content sentinel.');
  }
...
function renderSyntheticRoute(route) {
  ...
  for (const sentinel of route.includes ?? []) {
    if (routePath === '/references/' && sentinel === 'Medienfreigabe') continue;
