...
function buildReport(manifest, check) {
  ...
  if (contextReview.ifc_human_review_guide_ready && !contextReview.ifc_human_review_session_exists) {
    warnings.push('IFC human review guide is ready but design/ifc-human-review-session.json is missing.');
  }
  if (contextReview.ifc_human_review_session_status === 'ifc_human_review_session_positive_decision_blocked') {
...
function readContextReview() {
  ...
  const ifcHumanReviewSession = existsSync(ifcHumanReviewSessionPath) ? safeReadJson(ifcHumanReviewSessionPath) : null;
  ...
  return {
    ...
    ifc_human_review_viewer_exists: Boolean(ifcHumanReviewViewer),
    ifc_human_review_viewer_status: ifcHumanReviewViewer?.status || null,
    ifc_human_review_viewer_ready: isIfcHumanReviewViewerReady(ifcHumanReviewViewer),
    ifc_human_review_viewer_preview_count: numberOrDefault(ifcHumanReviewViewer?.summary?.preview_count, 0),
    ifc_human_review_viewer_open_human_check_count: numberOrDefault(ifcHumanReviewViewer?.summary?.open_human_check_count, 0),
    ifc_human_review_viewer_html: ifcHumanReviewViewer?.output_files?.html || null,
    ifc_human_review_decision_exists: Boolean(ifcHumanReviewDecision),
    ifc_human_review_decision_status: ifcHumanReviewDecision?.status || null,
    ifc_human_review_decision: ifcHumanReviewDecision?.decision || null,
    ifc_human_review_decision_final: Boolean(ifcHumanReviewDecision?.summary?.final_decision_recorded),
    ...
    ifc_human_review_session_exists: Boolean(ifcHumanReviewSession),
    ifc_human_review_session_status: ifcHumanReviewSession?.status || null,
    ifc_human_review_session_reviewer: ifcHumanReviewSession?.reviewer || null,
    ifc_human_review_session_proposed_decision: ifcHumanReviewSession?.proposed_decision || null,
    ...
    blender_context_smoke_exists: Boolean(blenderContextSmoke),
    blender_context_smoke_object_count: numberOrDefault(blenderContextSmoke?.object_count, 0),
    blender_context_smoke_locked_object_count: numberOrDefault(blenderContextSmoke?.locked_object_count, 0),
    blender_context_smoke_review_only_object_count: numberOrDefault(blenderContextSmoke?.review_only_object_count, 0),
...
function nextActions({ modules, gates, blockers, warnings, outputs, contextReview }) {
  ...
  if (contextReview?.ifc_human_review_session_exists && !contextReview.ifc_human_review_session_decision_ready) actions.push('Complete the IFC human review session checks before recording a final decision.');
...
function appendContextReview(lines, contextReview) {
  ...
  lines.push(`- IFC human review viewer: ${ifcHumanReviewViewerLabel(contextReview)}`);
  lines.push(`- IFC human review viewer previews: ${contextReview.ifc_human_review_viewer_preview_count}`);
  lines.push(`- IFC human review viewer open human checks: ${contextReview.ifc_human_review_viewer_open_human_check_count}`);
  lines.push(`- IFC human review viewer html: ${contextReview.ifc_human_review_viewer_html || '-'}`);
  lines.push(`- IFC human review decision: ${ifcHumanReviewDecisionLabel(contextReview)}`);
  lines.push(`- IFC human review decision value: ${contextReview.ifc_human_review_decision || '-'}`);
  lines.push(`- IFC human review decision final: ${contextReview.ifc_human_review_decision_final ? 'yes' : 'no'}`);
  ...
  lines.push(`- IFC human review session: ${ifcHumanReviewSessionLabel(contextReview)}`);
  lines.push(`- IFC human review session reviewer: ${contextReview.ifc_human_review_session_reviewer || '-'}`);
  lines.push(`- IFC human review session proposed decision: ${contextReview.ifc_human_review_session_proposed_decision || '-'}`);
  ...
  lines.push(`- Blender context smoke: ${blenderContextSmokeLabel(contextReview)}`);
  lines.push(`- Blender context smoke objects: ${contextReview.blender_context_smoke_object_count}`);
...
function isIfcHumanReviewViewerReady(viewer) {
  return Boolean(
    viewer
      && viewer.status === 'ifc_review_viewer_ready'
      && viewer.summary?.preview_count > 0
  );
}

function ifcHumanReviewViewerLabel(contextReview) {
  if (!contextReview.ifc_human_review_viewer_exists) return 'missing';
  if (contextReview.ifc_human_review_viewer_ready) return 'ready';
  return `pending (${contextReview.ifc_human_review_viewer_status || 'unknown'})`;
}

function ifcHumanReviewDecisionLabel(contextReview) {
...
function ifcHumanReviewSessionLabel(contextReview) {
  if (!contextReview.ifc_human_review_session_exists) return 'missing';
  return contextReview.ifc_human_review_session_status || 'unknown';
}
...
function blenderContextSmokeLabel(contextReview) {
  if (!contextReview.blender_context_smoke_exists) return 'missing';
  if (
    contextReview.blender_context_smoke_object_count > 0
    && contextReview.blender_context_smoke_locked_object_count === contextReview.blender_context_smoke_object_count
    && contextReview.blender_context_smoke_review_only_object_count === contextReview.blender_context_smoke_object_count
  ) {
    return 'passed';
  }
  return 'needs review';
