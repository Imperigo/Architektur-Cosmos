...
function buildQueue({ dataLaneSweep, nightLoop, localWorkerRunbook, assetSourceMap, pilotEvidence, choiceMatrix }) {
  const failures = [];
  ...
  const tasks = [
    ...
    task({
      ...
      ownerAction: false,
      sourceIndependent: true,
      command: 'npm run kosmo:asset-source-candidate-map',
      evidence: `asset candidates ${assetSourceMap.summary?.asset_lane_candidates ?? 0}, material ${assetSourceMap.summary?.material_library_candidates ?? 0}`
    }),
    task({
...
function task({ id, lane, actor, action, executableNow, ownerAction, sourceIndependent, command, evidence }) {
  return {
    id,
    lane,
    actor,
...
function renderMarkdown(queue) {
  ...
  lines.push('## Tasks');
  lines.push('');
  lines.push('| Task | Lane | Actor | Executable now | Owner action | Command | Evidence |');
  lines.push('| --- | --- | --- | --- | --- | --- | --- |');
  queue.tasks.forEach((item) => {
    lines.push(`| \`${item.id}\` | ${item.lane} | ${item.actor} | ${item.executable_now ? 'yes' : 'no'} | ${item.owner_action_required ? 'yes' : 'no'} | \`${escapePipe(item.command)}\` | ${escapePipe(item.evidence)} |`);
  });
  lines.push('');
