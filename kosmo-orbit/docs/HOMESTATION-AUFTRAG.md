...
| **Trainer-Contract** (v0.8.2 / P5, `docs/V082-SPEZ.md` §6.5; Empfänger v0.8.3 / P6, `docs/V083-SPEZ.md` §9) | KosmoTrain schnürt ein Trainingspaket (Manifest + JSONL, Browser-Download, kein Bridge-Endpunkt) nach `kosmo.lora-train/v1` (`packages/kosmo-contracts/src/lora-train.ts`) — Adapter, sha256-gehashte Datensatz-Dateien, Rezept-Referenz, visibility-Deckel. Die HomeStation **nimmt das `lora-train/v1`-Manifest entgegen** und startet den echten Unsloth-Lauf daraus; ein `LoraTrainBerichtV1` (verallgemeinerte Form von `LoraTrainBericht`) fasst das Ergebnis adapterbezogen zusammen. Der Empfänger-Teil ist jetzt container-seitig gebaut: `tools/homestation-bridge/kosmo_bridge/lora_empfaenger.py` prüft Schema-Version/Adapter-Enum/sha256 jeder Datei gegen das Manifest und bereitet ein Unsloth-Laufskript-Template (Platzhalter für GPU-Host/Basismodell) vor — läuft im Container nur mit explizitem `--fake-worker`, ohne Vortäuschen eines echten GPU-Laufs; der Unsloth-/GPU-Lauf selbst bleibt HomeStation-Grenze. |
