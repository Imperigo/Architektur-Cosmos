...
| 9 | **NEU (v0.7.3 D4, Stream S3):** die Blatt-Typografie «Zwei Stimmen» (Titel Lato Heavy versal, Messbares IBM Plex Mono mit Tabellenziffern, `derive/stilblatt.ts`s `SCHRIFT_TITEL`/`SCHRIFT_MESSBAR`) ist reine SVG/PDF-Darstellung — **DXF kennt keine Web-Fonts**. `dxf/export.ts` bleibt unverändert: Text-Entities (`TEXT`/`MTEXT`) tragen weiterhin den DXF-Standardstil (`STANDARD`, das CAD-Programm löst ihn gegen seine eigene lokale Schriftzuordnung auf) — dokumentierter, bewusster Rückfall, kein stiller Verlust. Betrifft NUR die Typografie-Wahl (Titel/Mono-Stimme); Bemassungswerte selbst (Zahlen, SIA-Hochzahl-Notation) sind im DXF ohnehin reiner Text ohne Font-Einbettung. | Rhino, Revit, Grasshopper (DXF-Weg) | `packages/kosmo-kernel/src/dxf/export.ts` unverändert (kein D4-Bezug); Beleg s. `docs/GOLDEN-WECHSEL-D4.md` |
