...
| Verschneidungsprioritäten | Höhere Priorität schneidet niedrigere — **über Bauteilgrenzen hinweg** (Wand↔Wand↔Decke), Skin-genau; plus Ebenen-Verschneidungsgruppen [H-IntersectionPriority, C] | Wandknoten: Gehrung/T-Stoss/Mehrfachknoten geometrisch gelöst (ROADMAP 15/21/35), Poché-Join trennt nach Umbau-Status (`derive/plan.ts`) — aber **keine prioritätsgesteuerte Schichtverschneidung** zwischen Wand und Decke oder ungleichen Aufbauten | ◐ — Knoten ja, Prioritäts-System nein; Lücke A1 in Abschnitt 3 |
...
| Publisher | Publikations-Sets: einmal definierte Baum-Auswahl → Batch PDF/DWG/IFC/BIMx mit Namensregeln [H-Navigator, K] | Export je Blatt/Plansatz: Plansatz-PDF A0–A4, SVG, DXF (ezdxf-verifiziert), IFC, GLB (`app/modules/publish/export-sheets.ts`, ROADMAP 11) | ◐ — Formate alle da, aber keine **gespeicherten Publikations-Sets** (welche Blätter, welches Format, welcher Dateiname) → Lücke A4 |
...
| Trace & Reference | Referenz-Unterlage (anderes Geschoss, Schnitt, DWG) halbtransparent unterlegen [K] | Splitscreen (4 synchrone Fenster), Splat-/IFC-Kontext-Layer, Skizzen-Overlay | ◐ — «anderes Geschoss blass unterlegen» fehlt und wäre im Alltag oft gefragt (Teil von A8) |
