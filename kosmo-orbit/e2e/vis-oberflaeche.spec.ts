 ...
 * (1) alle Bestands-Tests unten (Zoom/Fit/Overlap/Palette/Kuratier)
 *     sind Manuell-only-Chrome (`vis-zoom-*`/`vis-palette-*`/`vis-kuratier-*`
 *     existieren im Island-Modus nicht, s. `vis-island.
 *     spec.ts`s C-15-Beweis) — sie bleiben WÖRTLICH unverändert (kein
 *     Assertion-Zeile angefasst; K35 hat die drei Minimap-Tests später
 *     mitsamt dem Feature durch einen Nicht-Existenz-Beweis ersetzt),
 *     bekommen aber diesen datei-weiten
 ...
 * nichts») — die Minimap (Welle 3) ist ERSATZLOS entfernt, auch im
 * Manuell-Chrome: die drei früheren Minimap-Tests dieser Datei (Sichtbarkeit
 * ab 5 Nodes · Klick verschiebt den Viewport · Toggle-Knopf) sind mit dem
 * Feature gefallen; dieser Nicht-Existenz-Beweis ersetzt sie. Der Canvas-
 * Testpunkt (30,30) — historischer Vertrag aus Stream F — bleibt geprüft.
 */
test('K35: keine Minimap mehr im Manuell-Chrome — Toggle und Karte existieren auch bei 12 Nodes nicht', async ({ page }) => {
  await oeffneVis(page);
  await page.click('[data-testid="drei-stimmungen"]');
  await expect(page.locator('[data-testid="vis-node-render"]')).toHaveCount(3);

  // Die «Drei Stimmungen»-Kette bringt 12 Nodes mit — weit über der
  // früheren 5-Node-Default-Schwelle: weder Karte noch Toggle noch das
  // frühere Dock-Panel erscheinen.
  await expect(page.locator('[data-testid="vis-minimap"]')).toHaveCount(0);
  await expect(page.locator('[data-testid="vis-minimap-viewport"]')).toHaveCount(0);
  ...
  // Historischer Canvas-Testpunkt (30, 30) bleibt frei/klickbar.
  await page.locator('[data-testid="node-canvas"]').click({ position: { x: 30, y: 30 } });
});
