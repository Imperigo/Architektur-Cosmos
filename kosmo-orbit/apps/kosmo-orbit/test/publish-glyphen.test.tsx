...
import { PUBLISH_GLYPHEN } from '../src/modules/publish/island/publish-glyphen';
...
describe('publish-glyphen: Bauvorschrift je Icon (werkzeug-icons.tsx:1-31, 24er-Norm)', () => {
  const NAMEN = Object.keys(PUBLISH_GLYPHEN);

  it('jedes Icon rendert ein SVG mit viewBox 0 0 24 24, strokeWidth 1.75, runden Kappen/Joins, aria-hidden', () => {
    for (const name of NAMEN) {
      const Icon = PUBLISH_GLYPHEN[name]!;
  ...
  it('`size` steuert die gerenderte Kantenlänge, die ViewBox bleibt 24', () => {
    const Icon = PUBLISH_GLYPHEN.zoom!;
